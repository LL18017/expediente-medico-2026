    ...
    <q-card v-else-if="consulta" class="main-block-card shadow-10 rounded-borders-lg overflow-hidden">
      ...
      <div class="q-pa-md flex column wrapper-content">
        ...
        <div class="row q-col-gutter-md q-mb-md">
          ...
          <div class="col-12 col-md-4">
            <div class="info-box bg-blue-50 q-pa-md rounded-borders h-100 border-light">
              ...
              <div class="text-body2 text-weight-bold text-black text-uppercase">{{ consulta.paciente.nombre }} {{
                consulta.paciente.apellidos }}</div>
              <div class="text-caption text-grey-9 q-mt-xs">{{ getEdad(consulta.paciente.fecha_nacimiento) }} años |
                Nac: {{ formatFechaCorta(consulta.paciente.fecha_nacimiento) }}</div>
              <div class="text-caption text-grey-9">Doc: {{ consulta.paciente.doc.tipo }} {{ consulta.paciente.doc.valor
              }}</div>
            </div>
          </div>
          ...
          <div class="col-12 col-md-4">
            <div class="info-box bg-teal-50 q-pa-md rounded-borders h-100 border-light">
              ...
              <div class="text-body2 text-weight-bold text-black text-capitalize">{{ formatFecha(consulta.fecha) }}
              </div>
              <div class="text-caption text-grey-9 q-mt-xs">Nº Expediente: <strong>{{ consulta._id }}</strong></div>
              <div class="text-caption text-grey-9">Peso: {{ consulta.peso }}kg | Alt: {{ consulta.estatura }}m | PA: {{
                consulta.presionarterial }}</div>
            </div>
          </div>
          ...
          <div class="col-12 col-md-4">
            <div class="info-box bg-orange-50 q-pa-md rounded-borders h-100 border-light">
              ...
              <div class="text-caption text-grey-10 q-mb-xs lines-2" :title="consulta.motivo"><strong>Motivo:</strong>
                {{ consulta.motivo }}</div>
              <div class="text-caption text-grey-10 lines-2" :title="consulta.diagnostico"><strong>Dx:</strong> {{
                consulta.diagnostico }}</div>
            </div>
          </div>
        </div>
        ...
        <div class="flex column receta-container">
          <div
            class="text-subtitle1 text-weight-bold text-green-9 q-mb-sm flex items-center border-bottom-light q-pb-xs">
            ...
            <div v-if="consulta.recetas && consulta.recetas.length > 0" class="row q-col-gutter-sm">
              <div class="col-12 col-md-6" v-for="(receta, index) in consulta.recetas" :key="index">
                <div class="bg-white border-light rounded-borders q-pa-sm row items-center medication-item shadow-1">
                  ...
                  <div class="col">
                    <div class="text-body2 text-weight-bold text-black">{{ receta.medicamento }} <span
                        class="text-grey-6 text-weight-regular float-right">#{{ receta.cantidad }}</span></div>
                    <div class="text-caption text-grey-9 q-mt-xs">{{ receta.dosis }} | {{ receta.frecuencia }} | {{
                      receta.durante }}</div>
                    <div v-if="receta.observaciones"
                      class="text-caption text-green-9 q-mt-xs bg-green-1 q-pa-xs rounded-borders"><em>{{
                        receta.observaciones }}</em></div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <!-- Footer de la Receta -->
        <div
          class="row justify-between items-end q-mt-md text-caption text-grey-7 print-footer border-top-light q-pt-sm">
          <div>
...
.main-block-card {
  ...
  overflow-y: auto;
  /* El scroll ahora sucede en todo el cuadro general */
}

/* Estilizar el scroll general */
.main-block-card::-webkit-scrollbar {
  width: 8px;
}

.main-block-card::-webkit-scrollbar-track {
  background: transparent;
}

.main-block-card::-webkit-scrollbar-thumb {
  background: #b0bec5;
  border-radius: 10px;
}

.main-block-card::-webkit-scrollbar-thumb:hover {
  background: #90a4ae;
}
