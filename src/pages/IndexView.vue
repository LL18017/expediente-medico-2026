      ...
      datasets: [
        {
          ...
          backgroundColor: "rgba(38, 166, 154, 0.7)", // más suave
          hoverBackgroundColor: "rgba(38, 166, 154, 1)",
          borderRadius: 6,
          barThickness: 8, // más delgado
        },
      ],
    },
    options: {
...
/* WATCH */
watch(monthlyPatientsSeries, () => {
  renderChart();
});

/* LOAD DATA */
onMounted(async () => {
  const ultimas = await getUltimasConsultas(10);

  proximasCitas.value = ultimas.map((c) => ({
